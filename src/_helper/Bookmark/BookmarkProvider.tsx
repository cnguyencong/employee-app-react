...
const BookmarkProvider = (props: any) => {
  ...
  const [mybookmarkData, setMybookmarkData] = useState<any>([]);
  ...
  const addNewBookmark = (bookmarkData: any, img_url: any) => {
    const bookmarkTemp = {
  ...
  const updatebookmark = (id: string, updatedData: any, img_url: string) => {
    const bookmarkTemp = {
      id: id,
    ...
    setBookmark(bookmark.map((item: any) => (item.id === id ? bookmarkTemp : item)));
  };

  const removebookmark = (id: string) => {
    setBookmark(bookmark.filter((data: any) => data.id !== id));
  };

  const mybookmark = (mybookmarkdata: any) => {
    setMybookmarkData((prev: any) => [...prev, mybookmarkdata]);
  };

  const updateMybookmark = (id: any, updateMydata: any, img_url: any) => {
    const bookmarkTemp = {
      id: id,
    ...
    setMybookmarkData(
      (mybookmarkData as any).map((item: any) => (item.id === id ? bookmarkTemp : item))
    );
  };



  const removemybookmark = (id: any) => {
    setMybookmarkData(mybookmarkData.filter((data: any) => data.id !== id));
  };
