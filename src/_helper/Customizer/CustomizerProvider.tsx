...
const CustomizerProvider = (props: any) => {
  ...
  const layoutValue: any = Object.values(defaultLayoutObj).pop();
  ...
  //Toggle sidebar
  const toggleSidebar = (toggle: any) => {
    setToggleIcon(toggle);
  };

  //Multiple Sidebar Layouts
  const addSidebarLayouts = (sidebar_layout: any) => {
  ...
  const addSidebarIconType = (sidebar_Icon_Type: any) => {
  ...
  const addSidebarSettings = (settings: any) => {
    ConfigDB.data.settings.sidebar_setting = settings;
  };

  //Add Mix layouts like (dark , light ,...)
  const addMixBackgroundLayout = (mix_background_layout: any) => {
  ...
  // Add Colors
  const addColor = (default_color: any, secondary_color: any) => {
  ...
  const toggleSidebarResponsive = (toggle: any) => {
