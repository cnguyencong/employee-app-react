...
const Leftbar = () => {
  const { layoutURL, toggleSidebar }: any = useContext(CustomizerContext);
  ...
  function useWindowSize() {
    ...
    useLayoutEffect(() => {
      function updateSize() {
        setSize((window as any).innerWidth);
        if (window.innerWidth <= 1007) {
          toggleSidebar(true);
          (document.querySelector('.page-wrapper') as any).className = 'page-wrapper compact-wrapper';
        } else {
          toggleSidebar(false);
        }
      }
  ...
  const responsive_openCloseSidebar = (toggle: any) => {
    if (+width <= 991) {
