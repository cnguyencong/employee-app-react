...
const AppLayout = ({ children, classNames, ...rest }: any) => {
  const { layout }: any = useContext(CustomizerContext);
  const { sidebarIconType }: any = useContext(CustomizerContext);

  const layout1 = localStorage.getItem('sidebar_layout') || layout;
  const sideBarIcon = localStorage.getItem('sidebar_icon_type') || sidebarIconType;
  const location = useLocation();
  const { animation }: any = useContext(AnimationThemeContext);
